			...
			using (var mutex = new Mutex(false, "Global\\lidgrenSocketBind"))
			{
				try
				{
                    ...
                    if (m_configuration.DualStack)
                    {
                        ...
                        {
                            //m_socket.DualMode = true; changed for NET 4.0 compatibility
                            m_socket.SetSocketOption(SocketOptionLevel.IPv6, (SocketOptionName)27, false);
                            m_socket.Bind(new NetEndPoint(IPAddress.IPv6Any, m_configuration.Port));
                        }
                    }
